...
from map import _unstack_pytree, _stack_pytree
...
def create_fw_bw_graph(cond_fn, body_fn, num_mapped_args, *args):
    ...
    with suspend_functionalization(), disable_functional_mode():
        with disable_proxy_modes_tracing():
            ...
            # )
            example_flat_out = pytree.tree_map(
                _from_fun, while_loop_op(cond_fn, body_fn, *example_xs, *example_pos_args)
            )
            if any(
            ...
            fw_graph = make_fx(while_loop_op)(cond_fn, body_fn, *example_xs, *example_pos_args)
        ...
        def joint_f(*example_args):
            ...
            def fw_with_masks(*args):
                fw_out = while_loop(cond_fn, body_fn, *args)
                return fw_out, [
                    True
...
def while_loop_autograd(cond_fn, body_fn, carried_inputs, additional_inputs):
    num_mapped_args = len(carried_inputs)
    fw_graph, bw_graph = create_fw_bw_graph(body_fn, num_mapped_args, *carried_inputs, *additional_inputs)
    flat_out = WhileLoopAutogradOp.apply(fw_graph, bw_graph, num_mapped_args, *carried_inputs, *additional_inputs)
    return flat_out
