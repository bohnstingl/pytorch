        ...
        def emit_node(node : Node, for_loop_cnt: int):
            ...
            if node.op == 'placeholder':
                ...
                if raw_name != repr(node):
                    body.append(intendation + f'{repr(node)} = {raw_name}\n')
                return
                ...
                if getattr(node.target, "__module__", "") == '_operator' and node.target.__name__ in inplace_methods:
                    body.append(intendation + f'{inplace_methods[node.target.__name__].format(*(repr(a) for a in node.args))};  '
                ...
                if global_name == 'getattr' and \
                   ...
                   len(node.args) == 2:
                    body.append(intendation + f'{repr(node)}{maybe_type_annotation} = {_format_target(repr(node.args[0]), node.args[1])}')
                    return
                body.append(intendation + f'{repr(node)}{maybe_type_annotation} = {global_name}({_format_args(node.args, node.kwargs)})')
                ...
                assert isinstance(node.target, str)
                body.append(intendation + f'{repr(node)}{maybe_type_annotation} = {_format_target(root_module, node.target)}')
                return
            elif node.op == 'output':
...
{code}"""
        ...
        if fn_code == to_patch:
            import pdb
            pdb.set_trace()
            #fn_code = '\n\n\ndef forward(self, s1 : torch.SymInt, s2 : torch.SymInt, L_input_ : torch.Tensor):\n    l_input_ = L_input_\n    getitem = l_input_[(0, slice(None, None, None))]\n    size = l_input_.size()\n    add_3 = l_input_[(0, slice(None, None, None))]\n    for i in range(0, 4):\n        getitem_4 = l_input_[(i, slice(None, None, None))]\n        add_3 = add_3 + getitem_4\n    getitem_4 = getitem = l_input_ = None\n    return (add_3,)'
            fn_code = ''
